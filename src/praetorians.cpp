...
const int SONIDO_DRAGON_VIVO = 30;
/* '''ALCOBAS REALES */
...
std::vector<int> PretorianAIOffset;
std::vector<int> PretorianDatNumbers;
/* ' */
/* ''Added by Nacho */
...
/* ' */
 ...
 void LoadPretorianData() {
 
 std::string PretorianDat;
 PretorianDat = DatPath + "Pretorianos.dat";
 
 int NroCombinaciones;
 NroCombinaciones = vb6::val(GetVar(PretorianDat, "MAIN", "Combinaciones"));
 
 PretorianDatNumbers.resize(0);
 PretorianDatNumbers.resize(1 + NroCombinaciones);
 ...
 /* ' KINGS */
 TempInt = vb6::val(GetVar(PretorianDat, "KING", "Cantidad"));
 PretorianAIOffset[ePretorianAI_King] = 1;
  for (Counter = (1); Counter <= (TempInt); Counter++) {
  
  /* ' Alto */
  PretorianDatNumbers[PretorianIndex] = vb6::val(GetVar(PretorianDat, "KING", "Alto" + vb6::CStr(Counter)));
  PretorianIndex = PretorianIndex+1;
  /* ' Bajo */
  PretorianDatNumbers[PretorianIndex] = vb6::val(GetVar(PretorianDat, "KING", "Bajo" + vb6::CStr(Counter)));
  PretorianIndex = PretorianIndex+1;
  
 }
 
 /* ' HEALERS */
 TempInt = vb6::val(GetVar(PretorianDat, "HEALER", "Cantidad"));
 PretorianAIOffset[ePretorianAI_Healer] = PretorianIndex;
  for (Counter = (1); Counter <= (TempInt); Counter++) {
  
  /* ' Alto */
  PretorianDatNumbers[PretorianIndex] = vb6::val(GetVar(PretorianDat, "HEALER", "Alto" + vb6::CStr(Counter)));
  PretorianIndex = PretorianIndex+1;
  /* ' Bajo */
  PretorianDatNumbers[PretorianIndex] = vb6::val(GetVar(PretorianDat, "HEALER", "Bajo" + vb6::CStr(Counter)));
  PretorianIndex = PretorianIndex+1;
  
 }
 
 /* ' SPELLCASTER */
 TempInt = vb6::val(GetVar(PretorianDat, "SPELLCASTER", "Cantidad"));
 PretorianAIOffset[ePretorianAI_SpellCaster] = PretorianIndex;
  for (Counter = (1); Counter <= (TempInt); Counter++) {
  
  /* ' Alto */
  PretorianDatNumbers[PretorianIndex] = vb6::val(GetVar(PretorianDat, "SPELLCASTER", "Alto" + vb6::CStr(Counter)));
  PretorianIndex = PretorianIndex+1;
  /* ' Bajo */
  PretorianDatNumbers[PretorianIndex] = vb6::val(GetVar(PretorianDat, "SPELLCASTER", "Bajo" + vb6::CStr(Counter)));
  PretorianIndex = PretorianIndex+1;
  
 }
 
 /* ' SWORDSWINGER */
 TempInt = vb6::val(GetVar(PretorianDat, "SWORDSWINGER", "Cantidad"));
 PretorianAIOffset[ePretorianAI_SwordMaster] = PretorianIndex;
  for (Counter = (1); Counter <= (TempInt); Counter++) {
  
  /* ' Alto */
  PretorianDatNumbers[PretorianIndex] = vb6::val(GetVar(PretorianDat, "SWORDSWINGER", "Alto" + vb6::CStr(Counter)));
  PretorianIndex = PretorianIndex+1;
  /* ' Bajo */
  PretorianDatNumbers[PretorianIndex] = vb6::val(GetVar(PretorianDat, "SWORDSWINGER", "Bajo" + vb6::CStr(Counter)));
  PretorianIndex = PretorianIndex+1;
  
 }
 
 /* ' LONGRANGE */
 TempInt = vb6::val(GetVar(PretorianDat, "LONGRANGE", "Cantidad"));
 PretorianAIOffset[ePretorianAI_Shooter] = PretorianIndex;
  for (Counter = (1); Counter <= (TempInt); Counter++) {
  
  /* ' Alto */
  PretorianDatNumbers[PretorianIndex] = vb6::val(GetVar(PretorianDat, "LONGRANGE", "Alto" + vb6::CStr(Counter)));
  PretorianIndex = PretorianIndex+1;
  /* ' Bajo */
  PretorianDatNumbers[PretorianIndex] = vb6::val(GetVar(PretorianDat, "LONGRANGE", "Bajo" + vb6::CStr(Counter)));
  PretorianIndex = PretorianIndex+1;
  
 }
 
 /* ' THIEF */
 TempInt = vb6::val(GetVar(PretorianDat, "THIEF", "Cantidad"));
 PretorianAIOffset[ePretorianAI_Thief] = PretorianIndex;
  for (Counter = (1); Counter <= (TempInt); Counter++) {
  
  /* ' Alto */
  PretorianDatNumbers[PretorianIndex] = vb6::val(GetVar(PretorianDat, "THIEF", "Alto" + vb6::CStr(Counter)));
  PretorianIndex = PretorianIndex+1;
  /* ' Bajo */
  PretorianDatNumbers[PretorianIndex] = vb6::val(GetVar(PretorianDat, "THIEF", "Bajo" + vb6::CStr(Counter)));
  PretorianIndex = PretorianIndex+1;
  
 }
 
 /* ' Last */
 PretorianAIOffset[ePretorianAI_Last] = PretorianIndex;
 
 /* ' Inicializa los clanes pretorianos */
 ClanPretoriano.resize(0);
 ClanPretoriano.resize(1 + 2);
 /* ' Clan default */
 ClanPretoriano[1].reset(new clsClanPretoriano());
 /* ' Invocable por gms */
 ClanPretoriano[2].reset(new clsClanPretoriano());
