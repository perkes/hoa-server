...
std::vector<uint8_t> SHA1_RawString(const std::string& str) {
	return SHA1_RawString(str.c_str(), str.size());
}
...
std::vector<uint8_t> SHA1_RawString(const char* data, size_t datalen) {
	...
	digest.resize(20, 0);
	SHA1_Init(&ctx);
	SHA1_Update(&ctx, reinterpret_cast<const uint8_t*>(data), datalen);
	SHA1_Final(&ctx, digest.data());
	return digest;
}

std::string SHA1_HexString(const std::string& str) {
	return SHA1_HexString(str.c_str(), str.size());
}

std::string SHA1_HexString(const char* data, size_t datalen) {
	auto digest = SHA1_RawString(data, datalen);
	std::string result;
	result.reserve(digest.size() * 2);

	for (auto x : digest) {
		char ss[10];
		snprintf(ss, sizeof(ss), "%02x", x);
		result += ss;
	}
